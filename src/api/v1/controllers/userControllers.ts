...
class userController {
	static async getUsers(req: any, res: any) {
		try {
			let users = await userHelper.getUsers(req.body);
	...
	static async signUp(req: any, res: any) {
		...
		try {
			...
			const table = `"Users"`;
			const user: any = await DB.insert(table, req.body, data);
			console.log('USER', user);
			const link = `http://localhost:3000/verify/${user.rows[0].id}`;
	...
	static async signOut(req: any, res: any) {
		try {
			const user =
				// res.clearCookie('jwt');
				responseHelper.success(res.clearCookie('jwt'), 'SIGNUP_SUCCESS', req.headers.language, '', '');
		} catch (error) {
			console.log(error);
			responseHelper.error(res, error, req.headers.language, '');
		}
	...
	static async getVerifyUser(req: any, res: any) {
		try {
			let id = req.params.id;
			let user = await userHelper.getVerifyUser(req.body, id);
			console.log('USER::', user);
